...
class Challenge(models.Model):
    ...
    participants = models.ManyToManyField(
        settings.AUTH_USER_MODEL, blank=True, related_name="participate_users"
    )
...
class ChallengeImage(models.Model):
    ...
    image = ProcessedImageField(upload_to=challenge_image_path, blank=True, null=True)
...
class Certification(models.Model):
    challenge = models.ForeignKey(
        Challenge, on_delete=models.CASCADE, related_name="certifications"
    )
    ...
    image = ProcessedImageField(
        upload_to=certification_image_path, blank=True, null=True
    )
