...
class S_Product(models.Model):
    ...
    like_users = models.ManyToManyField(
        settings.AUTH_USER_MODEL, related_name="like_s_products", blank=True
    )
...
class S_ProductImage(models.Model):
    ...
    image = ProcessedImageField(upload_to=s_product_image_path, blank=True, null=True)
...
class S_Purchase(models.Model):
    customer = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="s_purchases"
    )
    product = models.ForeignKey(S_Product, on_delete=models.CASCADE)
...
class S_Sales(models.Model):
    ...
    def get_total_sales_per_day(cls, date):
        total_sales = cls.objects.filter(date__date=date).aggregate(
    ...
    def get_total_sales_per_month(cls, year, month):
        total_sales = cls.objects.filter(date__year=year, date__month=month).aggregate(
            total_sales=Sum("amount")
        )
        return total_sales["total_sales"] or 0
